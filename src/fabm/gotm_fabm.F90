...
!  Observation indices (from obs_0d, obs_1d) for pelagic and benthic state variables.
   ...
   type (type_bulk_variable_id),      save :: temp_id,salt_id,rho_id,h_id,swr_id,par_id,pres_id,Hice_id
   type (type_horizontal_variable_id),save :: lon_id,lat_id,windspeed_id,par_sf_id,cloud_id,taub_id,swr_sf_id,HiceS_id
...
! !PRIVATE DATA MEMBERS:
   ...
   REALTYPE,pointer,dimension(:) :: nuh,h,bioshade,w,rho,Hice
...
!  GOTM-specific quantities
   ...
   REALTYPE,pointer :: I_0,A,g1,g2,lambda_ice,alb_ice
...
!BOC
   ...
   if (fabm_calc) then
      ...
      Hice_id = model%get_bulk_variable_id(standard_variables%Hice)
      HiceS_id       = model%get_horizontal_variable_id(standard_variables%HiceS)
...
!BOC
   ...
   call light(nlev,bioshade_feedback,Hice) !Hice
...
!
! !INTERFACE:
   subroutine set_env_gotm_fabm(latitude,longitude,dt_,nlev,w_adv_method_,w_adv_ctr_,temp,salt_,rho_,nuh_,h_,w_, &
                                bioshade_,I_0_,cloud,taub,wnd,precip_,evap_,z_,Hice_,alb_ice_,lambda_ice_,A_,g1_,g2_, &
...
! !INPUT PARAMETERS:
   ...
   integer,  intent(in) :: w_adv_method_,w_adv_ctr_,nlev
   REALTYPE, intent(in),target,dimension(:) :: temp,salt_,rho_,nuh_,h_,w_,bioshade_,z_,Hice_
   REALTYPE, intent(in),target :: I_0_,cloud,wnd,precip_,evap_,taub
   REALTYPE, intent(in),target :: A_,g1_,g2_,alb_ice_,lambda_ice_
...
!BOC
   ...
   call fabm_link_bulk_data      (model,Hice_id,     Hice_)
   call fabm_link_horizontal_data(model,HiceS_id,    Hice_(nlev))
...
!
! !INTERFACE:
   subroutine light(nlev,bioshade_feedback,Hice) !Hice
!
! !DESCRIPTION:
...
! !INPUT PARAMETERS:
   ...
   REALTYPE, intent(in),dimension(:)   :: Hice
   
!
! !REVISION HISTORY:
!  Original author(s): Jorn Bruggeman
!  Revised by Francois Clayer to include Ice shading
!
! !LOCAL VARIABLES:
   integer :: i
   REALTYPE :: bioext
   REALTYPE :: localexts(1:nlev)
   REALTYPE :: alb, ice_att_coeff
!EOP
!-----------------------------------------------------------------------
!BOC
   bioext = _ZERO_
   alb = _ZERO_
   ice_att_coeff = _ZERO_
   ...
   call fabm_get_light_extinction(model,1,nlev,localexts(1:nlev))
      ice_att_coeff = 1 
      alb = 0
      ! Include the albedo effect of ice when ice cover
      if (Hice(nlev) .gt. 0) then
              alb = alb_ice
   ...
   do i=nlev,1,-1
      ...
      par(i) = I_0*(1-alb)*(1-A)*exp(-z(i)/g2-bioext)*ice_att_coeff
      swr(i) = par(i)+I_0*(1-alb)*A*exp(-z(i)/g1)*ice_att_coeff
