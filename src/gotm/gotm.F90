...
!  This sets pointers, rather than copying data, and therefore needs to be done only once.
   ...
   call set_env_gotm_fabm(latitude,longitude,dt,nlev,w_adv_method,w_adv_discr,t(1:nlev),s(1:nlev),rho(1:nlev), &
                          nuh,h,w,bioshade(1:nlev),I_0,cloud,taub(1),wind,precip,evap,z(1:nlev), &
                          Hice(1:nlev),alb_ice,lambda_ice,A,g1,g2,yearday,secondsofday,SRelaxTau(1:nlev),sProf(1:nlev), &
...
!  Accept the current biogeochemical state and used it to compute derived diagnostics.
   ...
   call do_air_sea(julianday,secondsofday,nlev,Hice)
...
!     update temperature and salinity
      ...
      if (t_prof_method .ne. 0) then
         call temperature(nlev,dt,cnpar,I_0,heat,nuh,gamh,rad,Hice,h_frazil)
      endif
