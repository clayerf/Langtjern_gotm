...
!variables fot ice
   REALTYPE, public, dimension(:), allocatable  :: Hice,h_frazil
!  a dummy array
...
!  the hypsograph at the interfaces Af
   ...
   !These parameters are added for our homegrown ice module
   REALTYPE, public                              :: temp_ice
   REALTYPE, public                              :: alb_ice
   REALTYPE, public                              :: thresh_ice
   REALTYPE, public                              :: lambda_ice
   REALTYPE, public                              :: avmolt_ice
!
! !DEFINED PARAMETERS:
...
! !LOCAL VARIABLES:
   ...
   namelist /meanflow/  h0b,z0s_min,charnock,charnock_val,ddu,ddl,     &
                        ...
                        hypsograph_file,water_balance_method,temp_ice, &
                        alb_ice, thresh_ice, lambda_ice, avmolt_ice
!
!-----------------------------------------------------------------------
!BOC
...
!  Initialize cumulative run time used to detect u and v ramp.
   ...
   allocate(Hice(0:nlev),stat=rc)
   if (rc /= 0) STOP 'init_meanflow: Error allocating (Hice)'
   Hice = _ZERO_

   allocate(h_frazil(0:nlev),stat=rc)
   if (rc /= 0) STOP 'init_meanflow: Error allocating (h_frazil)'
   h_frazil = _ZERO_
...
!BOC
   ...
   if (allocated(h_frazil)) deallocate(h_frazil)
...
!BOC
   ...
   if (allocated(Hice))   LEVEL2 'Hice',Hice
   if (allocated(h_frazil))   LEVEL2 'h_frazil',h_frazil
