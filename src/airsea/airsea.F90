...
!  (m/s)
   ...
   REALTYPE, public, target            :: precip_ice
...
!
! !INTERFACE:
   subroutine do_air_sea(jul,secs,nlev,Hice)
!
! !DESCRIPTION:
!
...
! !INPUT PARAMETERS:
   ...
   REALTYPE, intent(in)                :: Hice(0:nlev)
!
! !REVISION HISTORY:
!  Original author(s): Karsten Bolding
!
...
!     Calculate bulk fluxes from meteorological conditions and surface state (sst,ssu,ssv).
      call flux_from_meteo(jul,secs,nlev,Hice)
...
!
! !INTERFACE:
   subroutine flux_from_meteo(jul,secs,nlev,Hice)
!
! !DESCRIPTION:
...
!BOC
   ...
   LEVEL2 'heat',heat
   LEVEL2 'tx,ty',tx,ty
   LEVEL2 'precip,precip_ice,evap',precip,precip_ice,evap
