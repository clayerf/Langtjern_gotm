...
! !PUBLIC_DERIVED_TYPES:
  type,extends(type_base_model),public :: type_selma
      ...
      type (type_dependency_id)            :: id_temp,id_salt, id_Hice
      type (type_horizontal_dependency_id) :: id_taub,id_wind, id_HiceS
...
! !INPUT PARAMETERS:
   ...
   call self%register_dependency(self%id_Hice, standard_variables%Hice)
   call self%register_dependency(self%id_HiceS, standard_variables%HiceS)
...
! !INPUT PARAMETERS:
  ...
  real(rk)           :: temp,wnd,salt,o2,hice,hiceS
  real(rk),parameter :: secs_per_day = 86400._rk
!
! !LOCAL VARIABLES:
...
!
   ...
   _GET_(self%id_o2,o2)
   _GET_(self%id_Hice,hice)
   _GET_HORIZONTAL_(self%id_HiceS,hiceS)
