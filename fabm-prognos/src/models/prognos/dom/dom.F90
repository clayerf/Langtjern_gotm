...
! !PUBLIC DERIVED TYPES:
   type,extends(type_base_model),public :: type_prognos_dom
...
!KB      procedure :: get_light_extinction
   end type
!EOP
!-----------------------------------------------------------------------
...
!
! !INPUT PARAMETERS:
   class (type_prognos_dom), intent(inout), target :: self
...
!BOC
   ...
   ! Register dependencies on external state variables
           ...
           ! Register the contribution of all state variables to total CDOM
...
!   call self%register_dependency(self%id_I_0,  standard_variables%surface_downwelling_photosynthetic_radiative_flux)!standard_variables%surface_downwelling_photosynthetic_radiative_flux)
   call self%register_dependency(self%id_temp, standard_variables%temperature)
   !ergom dependencies   
!   STDERR 'Greetings from the DOM module'
#endif
...
! !INPUT PARAMETERS:
   ...
   class (type_prognos_dom),intent(in) :: self
...
!   real(rk)                   :: Kd_DOC !light extinction coefficient dependent on DOC concentration
!   real(rk)                   :: scaled_kOM1, scaled_kOM2 !scaled parameters
...
!EOP
   ...
   integer :: cnt = 0
   !OPEN(UNIT=1,FILE="mat.txt",FORM="FORMATTED")
...
! The code below is not compiled for now - but can be used for inspiration
#if 1
